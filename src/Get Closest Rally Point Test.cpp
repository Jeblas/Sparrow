...
coordinate currentCoor = {37.198430, -80.575874, 10};
				...
				if (mavlink_parse_char(MAVLINK_COMM_0, rx_buffer[i], &recv_msg, &status)) {
					switch (recv_msg.msgid) {
						...
						case MAVLINK_MSG_ID_GLOBAL_POSITION_INT:
							// std::cout << "Lat: " << mavlink_msg_global_position_int_get_lat(&recv_msg) * 1E-7<< '\n';
							// std::cout << "Lon: " << mavlink_msg_global_position_int_get_lat(&recv_msg) * 1E-7<< '\n';
							// std::cout << "Alt: " << mavlink_msg_global_position_int_get_lat(&recv_msg) * 1E-7<< '\n';
...
// Main loops 
    ...
    coordinate * gotoLocation = {};
    gotoLocation = getClosestRallyPoint();
    std::cout << std::fixed;
    std::cout << std::setprecision(8);
    std::cout << "Goto location Lat: " << gotoLocation->lat << '\n';
    std::cout << "Goto location Lon: " << gotoLocation->lon << '\n';
// Clean up
	// scan.join();
	// search.join();
	// endConnection();
