...
void sendDoReposition(coordinate * destination) {
	...
	int32_t latitude = destination->lat * 1E7;
	int32_t longitude = destination->lon * 1E7;
	float altitude = currentCoor.alt * 1E3;
	...
	// Might need to change this to command_int (#75)
...
void handleTrigger() {
	...
	// TODO check logic and remove debug lines
				...
				if (mavlink_parse_char(MAVLINK_COMM_0, rx_buffer[i], &recv_msg, &status)) {
					switch (recv_msg.msgid) {
						case MAVLINK_MSG_ID_HEARTBEAT:
							...
							if (!isTriggered && heartbeat.custom_mode == 131072) {
								isTriggered = true;
